...
)
{
	...
	while (ndata) {	/* Process all tables in the segment */
		if (ndata < 65)
		{
			TJDG_ADDERRORINFO("[HEADER] : Quantization table size(%d) too small",ndata);
		...
		if (d & 0xF0)
		{
			TJDG_ADDERRORINFO("[HEADER] : Quantization table isn't 8-bit resolution");
			return JDR_FMT1;			/* Err: not 8-bit resolution */
		}
		i = d & 3;								/* Get table ID */

		pb0 = (BYTE *)alloc_pool(jd, 64 * sizeof (BYTE));/* Allocate a memory block for the table */
		if (!pb0) return JDR_MEM1;				/* Err: not enough memory */
		jd->qttbl0[i] = pb0;					/* Register the table */
		...
		for (i = 0; i < 64; i++) {				/* Load the table */
			...
			pb[z] = (LONG)((DWORD)t * IPSF(z));	/* Apply scale factor of Arai algorithm to the de-quantizers */
			pb0[i] = t;							/* Store table raw data */
		}
	}
...
)
{
	...
	while (ndata) {	/* Process all tables in the segment */
		if (ndata < 17)
		{
			TJDG_ADDERRORINFO("[HEADER] : Huffman code table size(%d) too small",ndata);
		...
		if (d & 0xEE)
		{
			TJDG_ADDERRORINFO("[HEADER] : Invalid huffman code table calss(%x) or number(%x)",cls,num);
		...
		if (ndata < np)
		{
			TJDG_ADDERRORINFO("[HEADER] : Invalid huffman code table");
			return JDR_FMT1;	/* Err: wrong data size */
		}
		ndata -= np;
	...
	do {
		if (!msk) {				/* Next byte? */
			...
			if (f) {			/* In flag sequence? */
				...
				if (*dp != 0)
				{
					TJDG_ADDERRORINFO("[BITSTREAM] : Invalid huffman code : FF %X",*dp);
...
)
{
	...
	TJDG_ADDERRORINFO("[BITSTREAM] : Invalid huffman code");
	...
	for (blk = 0; blk < nby + nbc; blk++) {
		...
		do {
			...
			if (z) {
				...
				if (i >= 64)
				{
					TJDG_ADDERRORINFO("[BITSTREAM] : Invalid REL code");
		...
		if (JD_USE_SCALE && jd->scale == 3)
			*bp = (BYTE)((*tmp / 256) + 128);	/* If scale ratio is 1/8, IDCT can be omitted and only DC element is used */
		else
...
)
{
	...
	rect.left = (WORD)x; rect.right = (WORD)(x + rx - 1);				/* Rectangular area in the frame buffer */
	rect.top = (WORD)y; rect.bottom = (WORD)(y + ry - 1);
...
)
{
	...
	{
		TJDG_ADDERRORINFO("[BITSTREAM] : DRI error,expected 0xFFD%X,but find 0x%04X",(rstn & 7),d);
...
)
{
	...
	jd->pool = jd->buffer;		/* Work memroy */
	jd->sz_pool = sizeof(jd->buffer);	/* Size of given work memory */
	jd->infunc = infunc;	/* Stream input function */
	jd->device = dev;		/* I/O device identifier */
	jd->nrst = 0;			/* No restart interval (default) */
	jd->used_byte = 0;      /* used byte is zero */
	...
	for (i = 0; i < 4; i++) {jd->qttbl[i] = 0;jd->qttbl0[i] = 0;}
	...
	{
		TJDG_ADDERRORINFO("[HEADER] : SOI is not detected");
	...
	for (;;) {
		...
		{
			TJDG_ADDERRORINFO("[HEADER] : Invalid marker");
			return JDR_FMT1;
		}
		...
		switch (marker & 0xFF) {
		case 0xE0:  /* APP0 (baseline JPEG) */
			if (len > JD_SZBUF)
			{
				TJDG_ADDERRORINFO("[HEADER] : Masker(FF%2X) size(%d) too large,no memory for it",marker & 0xFF,len);
				return JDR_MEM2;
			}
			if (jd->infunc(jd, seg, len) != len) return JDR_INP;
			jd->used_byte += len;

			if(strncmp((const char *)seg,"JFIF",5))
			{
				TJDG_ADDERRORINFO("[HEADER] : Unkown jpeg format : %4s",seg);
				return JDR_FMT1;
			}

			if((seg[5] > 0x01))
			{
				TJDG_ADDERRORINFO("[HEADER] : Unkown jpeg vision : %d.%d",seg[5],seg[6]);
				return JDR_FMT1;
			}
		...
		case 0xDA:	/* SOS */
			...
			{
				TJDG_ADDERRORINFO("[HEADER] : Invalid image size");
			...
			for (i = 0; i < 3; i++) {
				...
				{
					TJDG_ADDERRORINFO("[HEADER] : Invalid huffman table for component %d",i);
					return JDR_FMT1;							/* Err: Huffman table not loaded */
				}
				if (!jd->qttbl[jd->qtid[i]])
				{
					TJDG_ADDERRORINFO("[HEADER] : Invalid dequantizer table for component %d",i);
			...
			if (!n)
			{
				TJDG_ADDERRORINFO("[HEADER] : Couldn't find SOF0");
...
)
{
	...
	if (dc < 16) {	/* No input data is available, re-fill input buffer */
		dp = jd->inbuf;	/* Top of input buffer */
		dc = jd->infunc(jd, dp, 16 - dc);
		if (!dc) return JDR_INP;	/* Err: read error or wrong stream termination */
	}
	...
	for(i = 0;i < 16;i++)
	{
		...
		else if(dp[i] == 0xFF)
		{
			...
			{
				TJDG_ADDERRORINFO("[BITSTREAM] : EOI miss");
				rc = JDR_FMT1;	/* Err: EOI is not detected */
				break;
			}
			else
			{
...
/*-----------------------------------------------------------------------*/
/* Check completeness of the JPEG picture                                */
/*-----------------------------------------------------------------------*/

JRESULT jd_fastcheck(	JDEC* jd,			/* Blank decompressor object */
						UINT (*infunc)(JDEC*, BYTE*, UINT),	/* JPEG strem input function */
						void* dev			/* I/O device identifier for the session */
					)
{
	...
	res = jd_prepare(jd,infunc,dev);

	if(res == JDR_OK)
	{
		res = jd_decomp(jd,NULL,3);
	}
...
/*-----------------------------------------------------------------------*/
/* Get error infomation decompress the JPEG picture                      */
/*-----------------------------------------------------------------------*/

const char *jd_geterrinfo(JDEC* jd,JRESULT rc)
{
	char * res = NULL;
	switch (rc)
	{
	...
	case JDR_INTR:
		{
			TJDG_ADDERRORINFO("[IO] : Output error");
			res = jd->errinfo;
			break;
		}
	case JDR_INP:
		{
			TJDG_ADDERRORINFO("[IO] : Input error");
			res = jd->errinfo;
			break;
		}
	case JDR_MEM1:
		{
			TJDG_ADDERRORINFO("[IO] : Not enough memory for work buffer");
			res = jd->errinfo;
			break;
		}
	case JDR_PAR:
		{
			TJDG_ADDERRORINFO("[PARA] : Parameter error");
			res = jd->errinfo;
			break;
		}
	case JDR_MEM2:
	...
	case JDR_FMT3:
		{
			TJDG_ADDERRORINFO("[HEADER] : Not supported JPEG standard");
			res = jd->errinfo;
			break;
		}
	case JDR_FMT2:
...
/*-----------------------------------------------------------------------*/
/* Get quantization table raw data of JPEG picture                       */
/*-----------------------------------------------------------------------*/

BYTE * jd_getqt(JDEC* jd,
				BYTE comp) /* Component number 0:Y, 1:Cb, 2:Cr */
{
	if(comp > 2)
		return NULL;

	return jd->qttbl0[jd->qtid[comp]];
}
